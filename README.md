...
> **注意：** 本库与达梦数据库 sqllog 的输出格式绑定，**不同版本的达梦数据库可能产生格式不同的日志文件**，字段顺序、分隔符或新增字段均可能导致解析失败或字段错位。本库不保证积极跟进格式变更，如遇解析异常请在 [Issues](https://github.com/guangl/dm-database-parser-sqllog/issues) 中附上脱敏的日志样本及达梦数据库版本。
